...
export async function getAuthStatus(): Promise<{ auth_enabled: boolean }> {
  return fetcher<{ auth_enabled: boolean }>(
...
export async function getImapFolders(): Promise<string[]> {
  return fetcher<string[]>(
    `${API_BASE_URL}/imap/folders`,
    {},
    'Failed to fetch IMAP folders',
    true,
  );
}
...
export async function startGmailOAuth2(
  redirectUri: string,
): Promise<{ authorization_url: string; state: string }> {
  const params = new URLSearchParams({ redirect_uri: redirectUri });
  return fetcher<{ authorization_url: string; state: string }>(
    `${API_BASE_URL}/auth/google/authorize?${params.toString()}`,
    {},
    'Failed to start Gmail OAuth2',
  );
}

export async function completeGmailOAuth2(
  code: string,
  redirectUri: string,
): Promise<GmailCredential> {
  const params = new URLSearchParams({ redirect_uri: redirectUri });
  return fetcher<GmailCredential>(
    ...
    'Failed to complete Gmail OAuth2',
  );
}

export async function listGmailCredentials(): Promise<GmailCredential[]> {
  return fetcher<GmailCredential[]>(
    `${API_BASE_URL}/auth/google/credentials`,
    {},
...
export async function revokeGmailCredential(
  ...
  return fetcher<{ message: string }>(
    `${API_BASE_URL}/auth/google/credentials/${encodeURIComponent(email)}`,
    { method: 'DELETE' },
